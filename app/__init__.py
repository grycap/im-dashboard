...
def create_app(oidc_blueprint=None):
    ...
    def authorized_with_valid_token(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if 'token' in session:
                ...
                import jwt  # set the import here as it is only needed for debug (PyJWT package)
        ...
        for input_name, input_params in template_inputs.items():
            ...
            if input_params.get("tag_type") == "secret" and has_secret:
                if form_data.get(input_name, "").startswith("ott://"):
                    continue
                token, path = ott.write_data(access_token, secret_value)
    ...
    def _get_image_and_nets(cred_data, cred_id, form_data):
        ...
        pub_network_id = None
        if cred_data['type'] in ['fedcloud', 'OpenStack', 'OpenNebula', 'Linode',
                                 'Orange', 'GCE', 'CH', 'EUNode', 'Chameleon']:
            ...
            if form_data.get('extra_opts.selectedImage', "") != "" and 'name' in site:
                image = "egi://%s/%s?%s" % (site['name'], form_data['extra_opts.selectedImage'], vo)
